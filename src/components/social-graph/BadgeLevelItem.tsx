...
export function BadgeLevelItem({ level, isActive }: BadgeLevelItemProps) {
  return (
    <View className="flex-row items-start py-4 border-b border-[#27272A]">
      {/* Icône de statut */}
      <View
        className={`w-8 h-8 rounded-full items-center justify-center mr-4 ${
          level.is_unlocked ? 'bg-[#10B981]' : 'bg-[#27272A]'
        }`}
      >
        {level.is_unlocked ? (
      ...
      <View className="flex-1">
        <View className="flex-row items-center mb-1">
          <Text
            className={`font-semibold text-base ${
              level.is_unlocked ? 'text-white' : 'text-[#52525B]'
            }`}
          >
            Niv. {level.level}
          </Text>
        ...
        <Text
          className={`text-sm mb-1 ${
            level.is_unlocked ? 'text-[#A1A1AA]' : 'text-[#52525B]'
          }`}
        >
          {level.xp_required === 0
            ? 'Niveau de départ'
        ...
        {level.reward && (
          <View className="flex-row items-center mt-2 bg-[#27272A] px-3 py-2 rounded-lg">
