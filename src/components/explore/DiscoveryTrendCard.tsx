...
export function DiscoveryTrendCard({ item, onPress, onInterested, onNotInterested, feedbackDisabled = false }: { item: DiscoveryItem; onPress: () => void; onInterested?: () => void; onNotInterested?: () => void; feedbackDisabled?: boolean }) {
  ...
  return (
    <View className="mr-3 w-60 rounded-2xl border p-4" style={{ backgroundColor: colors.card, borderColor: colors.border }}>
      ...
      {onInterested && onNotInterested ? <View className="mt-3 flex-row border-t pt-3" style={{ borderColor: colors.border }}><TouchableOpacity onPress={onInterested} disabled={feedbackDisabled || item.viewerFeedbackType === 'INTERESTED'} className="mr-3 min-h-10 flex-1 justify-center rounded-lg" style={{ backgroundColor: item.viewerFeedbackType === 'INTERESTED' ? `${colors.primary}18` : 'transparent' }} accessibilityRole="button" accessibilityState={{ selected: item.viewerFeedbackType === 'INTERESTED' }} accessibilityLabel={`Intéressé par ${item.title}`}><Text className="text-center text-xs font-bold" style={{ color: feedbackDisabled ? colors.textMuted : colors.primary }}>{item.viewerFeedbackType === 'INTERESTED' ? 'Intéressé' : 'Ça m’intéresse'}</Text></TouchableOpacity><TouchableOpacity onPress={onNotInterested} disabled={feedbackDisabled} className="min-h-10 flex-1 justify-center" accessibilityRole="button" accessibilityLabel={`Pas intéressé par ${item.title}`}><Text className="text-center text-xs font-bold" style={{ color: feedbackDisabled ? colors.textMuted : colors.textSecondary }}>Pas intéressé</Text></TouchableOpacity></View> : null}
    </View>
  );
}
