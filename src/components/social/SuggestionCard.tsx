    ...
    <View className="px-4 py-3 border-b border-[#27272A]">
      <View className="flex-row items-start">
        ...
        <View className="flex-1 ml-3">
          <TouchableOpacity onPress={onPress} activeOpacity={0.7}>
            ...
            {user.bio && (
              <Text className="text-[#A1A1AA] text-xs mt-1" numberOfLines={2}>
                {user.bio}
              </Text>
            )}
      ...
      <View className="flex-row gap-2 mt-3">
        ...
        {onDismiss && (
          <TouchableOpacity
            onPress={onDismiss}
            className="px-4 bg-[#27272A] py-2.5 rounded-full items-center"
            activeOpacity={0.8}
          >
            <Text className="text-white text-sm font-semibold">Ignorer</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
