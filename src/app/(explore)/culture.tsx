...
function StateCard({ onRetry }: { onRetry: () => void }) { const colors = useThemeStore((state) => state.colors); return <View className="mx-4 mt-5 rounded-2xl border p-4" style={{ backgroundColor: colors.card, borderColor: colors.border }}><Icon name="cloud-offline-outline" size={24} color={colors.primary} /><Text className="mt-2 font-bold" style={{ color: colors.text }}>Impossible de charger la culture</Text><Text className="mt-1 text-sm" style={{ color: colors.textSecondary }}>Vérifiez votre connexion puis réessayez.</Text><TouchableOpacity onPress={onRetry} className="mt-3 self-start"><Text className="font-bold text-[#EF4444]">Réessayer</Text></TouchableOpacity></View>; }
