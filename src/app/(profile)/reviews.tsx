...
export default function ReviewsScreen() {
  const router = useRouter(); const colors = useThemeStore((state) => state.colors); const { data: reviews, isLoading, refetch } = useUserReviews(); const isDemo = useAuthStore((state) => state.sessionMode?.startsWith('demo-') ?? false);
  ...
  return <SafeScreen><Header title="Mes avis" onBack={() => router.back()} />{isLoading ? <LoadingState /> : reviews?.length ? <FlatList data={reviews} keyExtractor={(item) => item.id.toString()} contentContainerStyle={{ padding: 16, paddingBottom: 100 }} renderItem={({ item }) => <UserReviewCard review={item} onPress={() => router.push(`/(places)/${item.place.id}`)} onDelete={isDemo ? undefined : () => remove(String(item.id))} />} /> : <EmptyState title="Aucun avis" message="Partagez votre expérience en laissant des avis" icon={<Ionicons name="star-outline" size={64} color={colors.textMuted} />} />}{<View className="absolute bottom-6 left-4 right-4"><Button label="Choisir un lieu à évaluer" onPress={() => router.push('/(explore)/places')} leftIcon={<Ionicons name="create-outline" size={20} color="#FFFFFF" />} /></View>}</SafeScreen>;
}
