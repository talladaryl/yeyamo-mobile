...
function ModuleCard({ label, icon, color, index, onPress }: { label: string; icon: string; color: string; index: number; onPress: () => void }) {
  ...
  return <Animated.View entering={FadeInDown.delay(index * 45).duration(350)} style={animated} className="mb-3 w-[48.5%]"><TouchableOpacity onPressIn={() => { scale.value = withSpring(0.96); }} onPressOut={() => { scale.value = withSpring(1); }} onPress={onPress} className="flex-row items-center rounded-2xl border p-3.5" style={{ backgroundColor: colors.card, borderColor: colors.border }}><View className="h-11 w-11 items-center justify-center rounded-xl" style={{ backgroundColor: `${color}18` }}><Icon name={icon} size={22} color={color} /></View><Text className="ml-3 flex-1 text-xs font-bold" style={{ color: colors.text }}>{label}</Text></TouchableOpacity></Animated.View>;
}
