...
export default function ArtworkBasicInformation() {
  const router = useRouter(); const colors = useThemeStore((state) => state.colors); const draft = useCreateStore((state) => state.artworkDraft); const setDraft = useCreateStore((state) => state.setArtworkDraft); const countryCode = useCountryStore((state) => state.selectedCountryCode ?? 'CM'); const artisan = useMyArtisan();
  const partnerId = draft.artisanPartnerId ?? artisan.data?.partnerId ?? '';
  const next = () => { if (!draft.title?.trim() || !draft.shortDescription?.trim() || !partnerId) { Alert.alert('Profil artisan requis', 'Créez ou complétez votre profil artisan avant de publier une œuvre.'); return; } setDraft({ artisanPartnerId: partnerId, countryCode }); router.push('/(create)/artwork/story'); };
